...
# =====================================================
def global_alignment(a: str, b: str, match=3, mismatch=-3, gap=-2):
    """
...
# =====================================================
def local_alignment(a: str, b: str, match=3, mismatch=-3, gap=-2):
    """
...
# =====================================================
def fitting_alignment(a: str, b: str, match=3, mismatch=-3, gap=-2):
    """
    Fitting alignment: short sequence (a) must fully align to a substring of (b).
    Query (a) must align completely, but can start anywhere in target (b).
    """
    n, m = len(a), len(b)
    F = np.zeros((n + 1, m + 1), dtype=int)
    
    # Initialize: target can start anywhere (no penalty)
    F[0, :] = 0
    # But query must align from start (accumulate gap penalty)
    for i in range(1, n + 1):
        F[i, 0] = F[i - 1, 0] + gap
...
# =====================================================
def affine_global(a: str, b: str, match=3, mismatch=-3, gap_open=-5, gap_extend=-1):
    """
